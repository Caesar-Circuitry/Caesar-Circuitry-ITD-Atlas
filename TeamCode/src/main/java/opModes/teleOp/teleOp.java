...
public class teleOp extends CommandOpMode {
  ...
  private ElapsedTime transferTimer, intakeTransferTimer;
  ...
  public void initialize() {
    ...
    intakeTransferTimer = new ElapsedTime();
    transferTimer.reset();
    intakeTransferTimer.reset();
  ...
  public void run() {
    ...
    if (followingPath) {
      ...
      robot.drivetrainSubsystem.follower.setTeleOpMovementVectors(
          ...
          (-gamepad1.right_stick_x * 3 / 4) * multiplier,
          true);
    }
    if (transfer) {
      if (intakeTransferTimer.milliseconds() > 800) {
        schedule(new intSetViperTransfer(robot.intakeSubsystem));
      }
  ...
  private void driverMapping() {
    ...
    driver
        ...
        .whenPressed(new InstantCommand(this::slowDown));
    driver
        .getGamepadButton(GamepadKeys.Button.LEFT_BUMPER)
        .whenReleased(new InstantCommand(this::speedUp));
    driver
  ...
  private void operatorMapping() {
    ...
    operator
        .getGamepadButton(GamepadKeys.Button.Y)
        .whenPressed(new scoringTransferSample(robot.outtakeSubsystem, robot.intakeSubsystem));
    operator
